...
void setup() {
  ...
  // Setup GPIO pins as input with pull-ups
...
void draw() {
  ...
  if (gameState == 0) {
    ...
    // Start when ALL FOUR GPIO buttons are held
...
void drawTrack() {
  ...
  fill(30);
  rectMode(CENTER);
  rect(0, 0, trackWidth, BAR_LENGTH_PIXELS * 4);
    ...
    for (TrackData.Hit hit : bar.hits) {
      ...
      switch (hit.state) {
        ...
        case TrackData.Hit.HIT_SUCCESS:
          ...
          float alphaFactor = max(0, 1 - timeSinceHit / NOTE_FADE_TIME_SECONDS);
